...
// TestCrossReferenceConvertorConvertTenantObjectToUpstreamObject tests the
// ConvertTenantObjectToUpstreamObject methods of CrossReferenceConverter.
func TestCrossReferenceConvertorConvertTenantObjectToUpstreamObject(t *testing.T) {
	tenant := "111111"
	originName := "mycrb"
	userName := "myuser"
	...
	c := NewCrossReferenceConverter(NewDefaultConvertor(NewOwnerReferenceTransformer(checkGroupKind)), NewClusterRoleBindingTransformer())
	err := c.ConvertTenantObjectToUpstreamObject(&crb, tenant, false)
	if err != nil {
		t.Errorf("Failed to convert tenant object to upstream object.")
	}

	if crb.Name != tenant+util.TenantIDSeparator+originName {
		t.Errorf("Unexpect name")
	}
...
// TestCrossReferenceConvertorConvertUpstreamObjectToTenantObject tests the
// ConvertUpstreamObjectToTenantObject methods of CrossReferenceConverter.
func TestCrossReferenceConvertorConvertUpstreamObjectToTenantObject(t *testing.T) {
	tenant := "111111"
	originName := "mycrb"
	userName := "myuser"
	groupName := "system:serviceaccounts:mygroup"
	...
	crb := rbacinternal.ClusterRoleBinding{
		...
		RoleRef: rbacinternal.RoleRef{
			Kind:     "ClusterRole",
			Name:     tenant + util.TenantIDSeparator + clusterRoleName,
			APIGroup: "rbac.authorization.k8s.io",
		},
	}

	c := NewCrossReferenceConverter(NewDefaultConvertor(NewOwnerReferenceTransformer(checkGroupKind)), NewClusterRoleBindingTransformer())
	err := c.ConvertUpstreamObjectToTenantObject(&crb, tenant, false)
	if err != nil {
		t.Errorf("Failed to convert tenant object to upstream object.")
	}

	if crb.Name != originName {
		t.Errorf("Unexpect name")
	}
