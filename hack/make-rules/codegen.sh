...
#
# ⛔ The CRD is not optional and not decorative: the upstream cluster only serves
# quota.kubezoo.io because cmd/clusterresourcequota installs this at startup, and
# without it the tenant controller's discovery check returns nil and no tenant
# quota is enforced at all.
#
# ⚠️ The note that used to sit here said the crd target "needs controller-gen" and
# was "stable as long as the owned API types do not change". Half right. The
# artifact existed all along -- pkg/apis/quota/zz.generated.crd.go, 222 lines
# stamped "Code generated by crd-gen. DO NOT EDIT." -- but crd-gen is not in this
# repository, so DO NOT EDIT was an instruction nobody could follow and nothing
# could regenerate. That is a worse state than missing: the file looks maintained.
#
# ⭐ So the transcription is gone. controller-gen writes the YAML, the package
# embeds it, and NewClusterResourceQuotaCRD decodes it -- one source, no
# hand-copied schema to drift, and verify-codegen now covers it.
# ⚠️ ONLY the quota group. Tenant carries kubebuilder markers too, but kubezoo
# serves tenant.kubezoo.io ITSELF as a built-in API -- verified against a running
# kubezoo with `kubectl api-resources --api-group=tenant.kubezoo.io` -- so a
# Tenant CRD would give a type that already has a home a second, empty one.
# Generating it and relying on nobody applying it is not a safeguard.
run_crd() {
  install_pinned controller-gen sigs.k8s.io/controller-tools/cmd/controller-gen \
    "${CONTROLLER_GEN_VERSION}"
  # Scoped to the quota group, and written INTO the package, because
  # pkg/apis/quota embeds it -- go:embed cannot reach outside its own directory.
  run_quiet "${BIN_DIR}/controller-gen" \
    crd \
    paths=./pkg/apis/quota/... \
    output:crd:dir="${WORK_ROOT}/pkg/apis/quota"
}
